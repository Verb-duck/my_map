...
void my_map<Key, Value>::set(const Key& key, const Value& value)
{
	...
	else
	{
		...
		{
			if (key == cell_now->key)				//с таким ключём уже существует,
...
bool my_map<Key, Value>::erase(const Key& key)
{
	...
	while (now_cell->key != key)						 //ищем нужную ячейку
	{
		previos_cell = now_cell;
		if (key > now_cell->key)						 //идём вправо
		{
	...
	if (flag_turn_right)								 //если повернули направо
	{
		...
		if (now_cell->cell_left != nullptr)				 //если есть левое ветвление от удаляемой ячейки
		{
...
void my_map<Key, Value>::clear()
{
	...
	if (go_left != nullptr)			//смещаемся влево до упора
	{
		cell_head = go_left;
		this->clear();
	}
	if (go_right != nullptr)			//смещаемся вправо,
	{
		cell_head = go_right;
		this->clear();
	}
